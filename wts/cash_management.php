...
function getDailyTotal($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
            ...
            SUM(fare + charge) as total_amount,
            SUM(CASE WHEN payment_method = '現金' THEN fare + charge ELSE 0 END) as cash_amount,
            SUM(CASE WHEN payment_method = 'カード' THEN fare + charge ELSE 0 END) as card_amount,
            SUM(CASE WHEN payment_method = 'その他' THEN fare + charge ELSE 0 END) as other_amount
        FROM ride_records 
        WHERE DATE(ride_date) = ?
    ");
    $stmt->execute([$date]);
...
function getMonthlySummary($pdo, $month) {
    $stmt = $pdo->prepare("
        SELECT 
            ...
            SUM(CASE WHEN payment_method = '現金' THEN fare + charge ELSE 0 END) as cash,
            SUM(CASE WHEN payment_method = 'カード' THEN fare + charge ELSE 0 END) as card
        FROM ride_records 
    ...
    <div class="container mt-4">
        ...
        <?php if (!empty($daily_sales) || $daily_total['total_rides'] > 0): ?>
            <div class="data-status">
                <h6 class="mb-1">
                    <i class="fas fa-check-circle me-2"></i>データ連携正常
                </h6>
                <small>
                    乗車記録システムと正常に連携されています。
                    選択日: <?= date('Y年m月d日', strtotime($selected_date)) ?>
                    （乗車記録: <?= $daily_total['total_rides'] ?? 0 ?>件）
                </small>
            </div>
        <?php else: ?>
            <div class="no-data-warning">
                <h6 class="mb-1">
                    <i class="fas fa-exclamation-triangle me-2"></i>データがありません
                </h6>
                <small>
                    選択した日付（<?= date('Y年m月d日', strtotime($selected_date)) ?>）の乗車記録が見つかりません。
                    他の日付を選択するか、先に乗車記録を登録してください。
                </small>
            </div>
        <?php endif; ?>
