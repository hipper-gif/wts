...
// 運転者一覧取得（正しいカラム名を使用）
try {
    $drivers_sql = "SELECT id, NAME as name FROM users WHERE is_driver = 1 AND is_active = 1 ORDER BY NAME";
...
// 車両一覧取得（正しいカラム名を使用）
try {
    $vehicles_sql = "SELECT id, vehicle_number, vehicle_name FROM vehicles WHERE is_active = 1 ORDER BY vehicle_number";
